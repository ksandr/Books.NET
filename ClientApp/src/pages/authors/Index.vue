...
export default {
  ...
  methods: {
    load() {
      ...
      let url = "/odata/authors" + (this.query ? `?$filter=contains(Search, '${encodeURIComponent(this.query.toUpperCase())}')&` : "?");
    ...
    search(q) {
      this.query = q;
      this.$router.push({name: "authors", params: {page: 1}, query: this.query ? {q: encodeURIComponent(this.query)} : null});
    },
    details(id) {
      this.$router.push({name: "authors-details", params: {id: id}});
    },
  },
};
</script>
